...
public class GUI extends JFrame implements Observer {
	...
	tableScroller.setBorder(BorderFactory.createTitledBorder(""));

        final JTextArea manuelInputField = new JTextArea();
        JButton addButton = new JButton("add signatures", iconCache.getImageIcon(IconCache.IconId.ADD));
        addButton.addActionListener(e -> {
            try{
                String input = manuelInputField.getText();
                controller.addSignaturesRaw(input, controller.getModel().getCurrentSystem());
                manuelInputField.setText("");
            }
            catch(Exception ex){
                JOptionPane.showMessageDialog(GUI.this, ex.getMessage(), "error", JOptionPane.WARNING_MESSAGE);
                LOGGER.warn("error while adding signatures manually.", ex);
            }
        });

        JPanel manualAddPanel = new JPanel(new BorderLayout());
        manualAddPanel.add(new JScrollPane(manuelInputField), BorderLayout.CENTER);
        manualAddPanel.add(addButton, BorderLayout.SOUTH);
        manualAddPanel.setBorder(BorderFactory.createTitledBorder(""));

        JTabbedPane centerPanel = new JTabbedPane();
        centerPanel.addTab("signatures", tableScroller);
        centerPanel.addTab("add manually", manualAddPanel);
...
//	helpButton.setMnemonic('h');
        ...
        optionsButton.addActionListener(e -> {
            configurationDialog.openCentered();
            initAlwaysOnTop();
        });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            try {
                ...
                LOGGER.warn("error while settings look and feel.", ex);
            }
        ...
        try {
            if (o == controller) {
                ...
                switch (event.getType()) {
                    case DOWNTIME:
                        infoPanel.setIconText("Downtime");
                        break;
                    case OFFLINE:
                        infoPanel.setIconText("Offline");
                        break;
                    case ERROR:
            ...
            LOGGER.warn("unable to set application icon.");
        }
    }
